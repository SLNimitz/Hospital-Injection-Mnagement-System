...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $pa_injection_batch_no = isset($_POST['']) ? $_POST['pa_injection_batch_no'] : ''; 
        // Update the record
        $stmt = $pdo->prepare('UPDATE patient SET id = ?, pa_id = ?, baby_name = ?, baby_age = ?, gender = ?, pa_address = ?, pa_phone_number = ?, taken_injection = ?, pa_injection_batch_no = ? WHERE id = ?');
        $stmt->execute([$id, $pa_id, $baby_name, $baby_age, $gender, $pa_address, $pa_phone_number, $taken_injection, $pa_injection_batch_no, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="pa_update.php?id=<?=$contact['id']?>" method="post">
